---
title: "SAMSI Undergraduate Research Workshop 2020"
...
# Start here
  ...
  where <- "AccentRecognition"

  filename <- dir(where)[-1]
  n        <- length(filename)

  # Set the media player from your operating system to play the audio
  
  setWavPlayer("C:/Program Files/Window Media Player/wmplayer.exe")

  #somesounds <- sample(sample(sample(n)))[1:9]

```

Let's now pick only a few of the soundtracks

```{R}

   somesounds <- c(78, 282, 278, 210, 329,  44,  15,  90, 194) 
  
   graphics.off()
   for (i in somesounds)
   {
    
    track <- readWave(paste0(where,'/',filename[i]))
    plot(track, main='')
    plot(track, main=substr(filename[i],1,4))
  
  }  
```

What do some of the sounds sound like?

```{R}
   
  fokoue <- readWave('fokoue-intro-sml-1.wav')
  plot(fokoue, main='')
  plot(fokoue, main='Fokoue, Ernest')
  play(load.wave('fokoue-intro-sml-1.wav'))
  
```

How about hearing some of the sounds we will be analyzing thoroughly?
...
```{R}
  ...
  for (i in somesounds)
  {
   ...
   track <- readWave(paste0(where,'/',filename[i]))
   plot(track, main=substr(filename[i],1,4))
   play(load.wave(paste0(where,'/',filename[i])))  
  }  
...
We can now upload the data segment for this lab and explore it in ample details

```{R}
  
  accent <- read.csv('accent-mfcc-data-1.csv', header=T)
  ...
  n; p
```
One of the most natural things to do is 

```{r}
  plot(x, col=3+as.numeric(y))
```

We can ever look at just a few variables
```{r}
plot(x[,1:3], col=3+as.numeric(y))
```


How about some PCA? Principal

```{r}
  z <- predict(princomp(x), x)[,1:2]
  plot(z, col=3+as.numeric(y), xlab=expression(z[1]), ylab=expression(z[2]))
  text(z, labels=as.numeric(y), pos=4)
  xy.pca <- data.frame(y, z)
  colnames(xy.pca) <- c('y', 'z1', 'z2')
```


How about a distribution of $Y$
...
What does a "US Speaker versus Non US Speaker" classification  look like
```{r}
  y.us <- droplevels(as.factor(ifelse(y=='US','US','Non US')))

  prop.table(table(y.us))

  barplot(prop.table(table(y.us)))
  
  plot(x, col=3+as.numeric(y.us))
...
Let's mess around more with UK vs US

```{r}
  id.anglo <- which(y=='US' | y =='UK')
  y.anglo  <- y[id.anglo]
  
  y.anglo <- as.factor(droplevels(y.anglo))
  
  prop.table(table(droplevels(y.anglo)))

  barplot(prop.table(table(droplevels(y.anglo))))
  
  as.factor(droplevels(y.anglo))
  
  x.anglo <- x[id.anglo,]
  plot(x.anglo, col=3+as.numeric(y.anglo))
```

What would a tree look like

```{r}
  library(rpart)
  library(rpart.plot)

  tree.accent <- rpart(language~., data=xy)
  prp(tree.accent)
```

Some unsupervised learning with mixtures of gaussians  

```{R}
  
  yhat.lda <-predict(lda(MFCC,y), MFCC)$class

  yhat.svm.C <- predict(ksvm(language~., data=xy, type='C-svc'), MFCC)
  yhat.svm.nu <- predict(ksvm(language~., data=xy, type='nu-svc'), MFCC) 
...
Now that we have a reasonably small size, we may perform some learning
...
```{r}
  ...
  aic.logit.xy <- step(logistic.xy, scope=~., direction='both', k=2, trace=0)
  summary(aic.logit.xy)
  
  bic.logit.xy <- step(logistic.xy, scope=~., direction='both', k=log(n), trace=0)
  summary(bic.logit.xy)
  
```

Focus on a single characteristic

```{R}

   par(mfrow=c(1,2))
   hist(xy$X10, prob=T)
   lines(density(xy$X10), lwd=2, col='red')
   boxplot(xy$X10)
   
   boxplot(X10~y, data=xy, col=2:3)
```
Let's see even more
...
```{R}
  ...
  for(j in 1:12)
  {
    boxplot(xy[,j+1]~y, data=xy, col=j+1+as.numeric(xy$y), 
            main=colnames(xy)[j+1], ylab=colnames(xy)[j+1],
            xlab='Language')
  }
...
Single Linear Binary Logistic Regression

```{R}
  log.acc <- glm(y~X10, data=xy, family=binomial(link='logit'))
  summary(log.acc)
```
...
Let's now focus on binary classification. First let's do US vs NonUS speakers

```{r}
  xy <- data.frame(y.us, x)
  colnames(xy)[1] <- 'y'
...
# 
  ...
  log.mod  <- glm(y~., data=xy[id.tr, ], family=binomial(link='logit'))
  ...
  # 4-Naive Bayes 
  library(naivebayes)
  naive.mod  <- naive_bayes(y~., data=xy[id.tr, ])
  ...
  tree.mod  <- rpart(y~., data=xy[id.tr, ])
  ...
  rf.mod    <- randomForest(y~., data=xy[id.tr, ])
  ...
  gp.mod    <- gausspr(y~., data=xy[id.tr, ], type='classification', kernel='rbfdot')
...
#  
  k.opt.cv <- 5 #max(which(cv.error==min(cv.error)))
...
```{R}
  ...
  xy <- read.csv('accent-mfcc-data-1.csv', header=T)
  
  pos      <- 1
  colnames(xy)[pos] <- 'y'
  xy[,pos] <-  as.factor(ifelse(xy[,pos]=='US',1,0))
  y        <- xy[, pos]
  x        <- xy[,-pos]
  ...
  test.err <- matrix(0, nrow=R, ncol=10)
  ...
  for(r in 1:R)
  {
    ...
    y.te.hat       <- knn(x[id.tr,], x[id.te,], y[id.tr], k=1, prob=TRUE)
    ind.err.te     <- ifelse(y.te!=y.te.hat,1,0)                      # Random variable tracking error. Indicator
...
```{r}
  te.err <- data.frame(test.err)
  Method<-c('LDA','QDA', 'LOG', 'NB', 'kNN','SVM','TREE', 'RF','BOOST','GP')
  colnames(te.err) <- Method
  boxplot(te.err)
```

How about the nice ggplot version

```{R}
  require(reshape2)
  ggplot(data = melt(te.err), aes(x=variable, y=value)) + geom_boxplot(aes(fill=variable))+
    labs(x='Method', y=expression(hat(R)[te](hat(f))))+
    theme(legend.position="none") 
```
We can also explore the accuracy
```{R}
  test.acc <- data.frame(1-test.err)
  Method<-c('LDA','QDA', 'LOG', 'NB', 'kNN','SVM','TREE', 'RF','BOOST','GP')
  colnames(test.acc) <- Method
  boxplot(test.acc)
```  
...
```{R}
  require(reshape2)
  ggplot(data = melt(test.acc), aes(x=variable, y=value)) + geom_boxplot(aes(fill=variable))+
...
#  
  aov.method <- aov(value~variable, data=melt(te.err))
  anova(aov.method)
  #summary(aov.method)
